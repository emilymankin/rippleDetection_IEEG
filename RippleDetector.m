        ...
        function results = runRippleDataMicro(obj, runData, fileNameResults)
            ...
            for iPatient = 1:nPatients
                ...
                for iArea = 1:nAreas
                    ...
                    for iChan = 1:nMicroChans
                        if resultsPerArea(iArea).nRipples(iChan) > obj.minNripples % remove channels with no detections from calculation
        ...
        function plotResultsRipplesData(obj, results, folderToSave)
            ...
            for iPatient = 1:nPatients
                ...
                for iChan = 1:nChans
                    ...
                    cmin = nanmin(nanmin(TFRSmallBefore(:)),nanmin(TFRSmallStim(:)));
                    cmax = nanmax(nanmax(TFRSmallBefore(:)),nanmax(TFRSmallStim(:)));
                    if isnan(cmin); cmin = 0; end
                    if isnan(cmax); cmax = inf; end
